...
3. **Provide justification appropriate to the claim type** (HARD GATE — cannot be skipped, but its currency matches the claim):
   ...
   - **Empirical/systems claims:** mechanistic reasoning (WHY should this work, causally?) + measurement design: what will be measured, threats to validity, the confound list and how each confound is controlled. The threats list is not freeform — walk the fixed bias surface (`reference/bias-frameworks.md` §2: selection, confounding, assignment, protocol deviation, missing data, measurement, analysis flexibility, selective reporting), one line each: how it could operate here, or why it can't. The claim's verbs must respect `PROBLEM.md`'s question type — a descriptive or associational question does not acquire causal claims here. Decorative equations that don't carry the argument are a defect, not rigor — the reviewer is instructed to flag mathiness.
