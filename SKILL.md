...
- **The escalation rule (trigger and constraint, both mechanical):** the rule triggers when the last 2 `kind: metric` entries in `search_log` share the same `varies` value and neither produced a `best_state` improvement. When triggered, the next `kind: metric` hypothesis MUST target a dimension not yet present among `search_log`'s `kind: metric` entries — or the path decision moves to pivot/conclude. A sub-slice of the stalled dimension (`attention-sparsity` inside `attention-pattern`) is the same dimension; "this tweak feels different" does not satisfy the rule — a genuinely new dimension does. A genuine `kind: understanding` iteration on the stalled dimension (error analysis on existing runs, no beat-baseline runs) remains legal — the `kind` audit, not the label, decides what counts. A stalled verdict whose last 2 metric entries varied *different* dimensions imposes no dimension constraint (the structural-response requirement below still applies).
- **Escalation composes with budgets; it never overrides them.** A fired trigger means the last two metric iterations produced no improvement, so the diminishing-returns rule has fired as well: the Phase 5 recommendation remains conclude, and a fresh-dimension Path A is presented as the alternative for the user (or their pre-recorded default) to choose.
